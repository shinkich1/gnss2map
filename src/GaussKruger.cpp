...
public:
  ...
  {
    // Declare parameters
    declare_parameter("p0", std::vector<double>(3, 0.0));
    declare_parameter("gnss0", std::vector<double>(3, 0.0));
    declare_parameter("p1", std::vector<double>(2, 0.0));
    declare_parameter("gnss1", std::vector<double>(2, 0.0));
    declare_parameter("a", 6378137.0);
    declare_parameter("F", 298.257222);
    declare_parameter("m0", 0.9999);
    declare_parameter("ignore_th_cov", 16.0);

    // Get parameters
    get_parameter("p0", p0_);
    get_parameter("gnss0", gnss0_);
    get_parameter("p1", p1_);
    get_parameter("gnss1", gnss1_);
    get_parameter("a", a_);
    get_parameter("F", F_);
    get_parameter("m0", m0_);
    get_parameter("ignore_th_cov", ignore_th_cov_);
...
private:
  ...
  double a_, F_, m0_, ignore_th_cov_, offset_z_;
  double alpha_[5], A_[6], A_bar_, S_bar_phi0_, kt_;
  Eigen::Matrix2d K_;
  ...
  // Initialize internal variables and calibration
  void initVariable() {
    for (int i = 0; i < 2; ++i) {
      gnss0_[i] *= M_PI / 180;
      gnss1_[i] *= M_PI / 180;
  ...
  // Setup ROS subscriptions and publications
  void initPubSub() {
    sub_gnss_ = create_subscription<sensor_msgs::msg::NavSatFix>(
      "gnss/fix", 2,
      std::bind(&GaussKruger::cbGnss, this, std::placeholders::_1)
    );
    pub_gnss_pose_ = create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>(
  ...
  void cbGnss(sensor_msgs::msg::NavSatFix::ConstSharedPtr msg) {
    ...
    if (covariance > ignore_th_cov_ || status == sensor_msgs::msg::NavSatStatus::STATUS_NO_FIX) {
      x = NAN, y = NAN, z = NAN;
    } else {
